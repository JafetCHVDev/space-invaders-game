...
pub use game_state::{
    Bullet, DataKey, Direction, EntityPosition, GameState, Health, Invader, InvaderType,
    LeaderboardEntry, Player, Ship, Velocity, GAME_HEIGHT, GAME_WIDTH, INVADER_COLS, INVADER_ROWS,
    MAX_LEADERBOARD_SIZE, MAX_USERNAME_LENGTH,
};
...
impl SpaceInvadersContract {
    ...
    /// Username is stored on-chain and displayed in leaderboard
    ///
    /// # Arguments
    /// * `player` - Player's wallet address
    /// * `username` - Chosen username (max 16 characters)
    pub fn register_player(env: Env, player: Address, username: String) -> bool {
        ...
        env.storage().persistent().set(&key, &player_data);
        true
    }
    ...
    pub fn submit_score(env: Env, player: Address, score: u32) -> bool {
        ...
        // Get player data (must be registered)
        let mut player_data: Player = match env.storage().persistent().get(&player_key) {
            Some(p) => p,
            None => return false,
        };
        ...
        for i in 0..filtered_leaderboard.len() {
            ...
            if new_leaderboard.len() < MAX_LEADERBOARD_SIZE {
                new_leaderboard.push_back(existing);
            }
        }

        // Add at end if not inserted yet and space available
        if !inserted && new_leaderboard.len() < MAX_LEADERBOARD_SIZE {
            new_leaderboard.push_back(entry);
        }

        env.storage()
            .persistent()
            .set(&DataKey::Leaderboard, &new_leaderboard);
    ...
    pub fn is_registered(env: Env, player: Address) -> bool {
        env.storage().persistent().has(&DataKey::Player(player))
    }
}
